...
- Before using any mutating tool (write_to_file, apply_diff, edit, execute_command, apply_patch, search_replace, edit_file), intent context must be established first. The system may auto-select an intent from .orchestration/active_intents.yaml when intent_id is not provided; if the user explicitly requests not to select intent, the operation must fail with a valid active intent error.
