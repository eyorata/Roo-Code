...
export async function postIntentSelection(ctx: HookContext): Promise<void> {
  const intentManager = new IntentManager(ctx.workspaceRoot);
  const store = new ArtifactStore(ctx.workspaceRoot);

  // Example: load context docs
  const contextFiles = [
    path.join(ctx.workspaceRoot, "ARCHITECTURE_NOTES.md"),
    path.join(ctx.workspaceRoot, "README.md"),
